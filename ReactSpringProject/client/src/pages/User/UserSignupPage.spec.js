...
describe("UserSignupPage", () => {
  ...
  const getuserSignupPage = () => (<BrowserRouter><UserSignupPage /> </BrowserRouter>);


  describe("Layout", () => {
    it("has header of Sign Up", () => {
      const { container } = render(getuserSignupPage(0));
      const header = container.querySelector("h1");
      expect(header).toHaveTextContent("Sign Up");
    });

    it("has input for display name", () => {
      const { queryByPlaceholderText } = render(getuserSignupPage(0));
  ...
  describe("Interactions", () => {
    ...
    const setupForSubmit = () => {
      const rendered = render(getuserSignupPage(0));
